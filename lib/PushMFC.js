...
var PushMFC = (function () {
        ...
        while ((change = model._push.changes.shift()) !== undefined) {
            ...
            switch (change.prop) {
                case "vs":
                    ...
                    this.assert.notStrictEqual(model._push.events[2 /* VideoStates */], undefined);
                    targetDevices[model._push.events[2 /* VideoStates */]] = true;
                ...
                case "vs2":
                    //Record the target device for this change
                    this.assert.notStrictEqual(model._push.events[1 /* OnOff */], undefined);
                    targetDevices[model._push.events[1 /* OnOff */]] = true;
                    //Build the string for this change
                ...
                case "rank":
                    //Record the target device for this change
                    this.assert.notStrictEqual(model._push.events[3 /* Rank */], undefined);
                    targetDevices[model._push.events[3 /* Rank */]] = true;
                    //Build the string for this change
                    title = "PM: " + model.nm;
                ...
                case "topic":
                    //Record the target device for this change
                    this.assert.notStrictEqual(model._push.events[4 /* Topic */], undefined);
                    targetDevices[model._push.events[4 /* Topic */]] = true;
                    //Build the string for this change
                    line += "Has changed her topic:\n\t" + change.after + "\n";
                    break;
                case "cdstart":
                    //Record the target device for this change
                    this.assert.notStrictEqual(model._push.events[5 /* CountdownStart */], undefined);
                    targetDevices[model._push.events[5 /* CountdownStart */]] = true;
                    line += change.message + "\n";
                    break;
                case "cdend":
                    //Record the target device for this change
                    this.assert.notStrictEqual(model._push.events[6 /* CountdownComplete */], undefined);
                    targetDevices[model._push.events[6 /* CountdownComplete */]] = true;
                    line += change.message + "\n";
                    break;
                default:
            ...
            if (this.options.hasOwnProperty(device)) {
                for (var modelId in this.options[device]) {
                    ...
                    this.options[device][modelId].forEach(function (deviceIden, item) {
                        ...
                        if (item === 0 /* All */) {
                            model._push.events[1 /* OnOff */] = deviceIden;
                            model._push.events[2 /* VideoStates */] = deviceIden;
                            model._push.events[3 /* Rank */] = deviceIden;
                            model._push.events[4 /* Topic */] = deviceIden;
                            model._push.events[5 /* CountdownStart */] = deviceIden;
                            model._push.events[6 /* CountdownComplete */] = deviceIden;
                        }
                        else {
    ...
    PushMFC.prototype.modelStatePusher = function (model, before, after) {
        if (before !== after) {
            var change;
            if (model._push.events[1 /* OnOff */] !== undefined) {
            ...
            if (model._push.events[2 /* VideoStates */] !== undefined) {
    ...
    PushMFC.prototype.modelRankPusher = function (model, before, after) {
        if (model._push.events[3 /* Rank */] !== undefined && before !== after && (before !== undefined || after !== 0)) {
    ...
    PushMFC.prototype.modelTopicPusher = function (model, before, after) {
        if (model._push.events[4 /* Topic */] !== undefined && before !== after && after !== undefined && after !== null && after !== "") {
            model._push.changes.push({ prop: "topic", before: before, after: after, when: moment() });
            model._push.pushFunc();
        }
        if (after !== before && (model._push.events[5 /* CountdownStart */] !== undefined || model._push.events[6 /* CountdownComplete */] !== undefined)) {
                    ...
                    if (model._push.countdown.decrementMap[i] >= minimumDecrements) {
                        if (model._push.countdown.exists) {
                            if (model._push.countdown.index !== i) {
                                ...
                                if (newNumbers[i] === 0) {
                                    if (model._push.countdown.exists && model._push.events[6 /* CountdownComplete */] !== undefined) {
                            ...
                            if (model._push.events[5 /* CountdownStart */] !== undefined) {
                                model._push.changes.push({
                                    prop: "cdstart",
    ...
    PushMFC.prototype.resetCountdown = function (model, newNumbers) {
        model._push.countdown = {
            ...
            numbers: newNumbers,
            index: -1,
            decrementMap: newNumbers.map(function () {
                return 0;
            })
        };
    };
