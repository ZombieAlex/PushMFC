...
the room topics.  It emits one of 3 events in the process:
            ...
            if (newNumbers.length === oldNumbers.length && newNumbers.length > 0) {
                ...
                if (tracker.hasCountdown) {
                    ...
                    if (newIndex !== -1 && newNumbers.lastIndexOf(oldNumbers[tracker.index]) === newIndex) {
                        // @TODO - Remove this debugging log line when we've validated this code...
                        //  log(`Updating countdown info for ${model.nm}
                        //  "${before}" (index: ${tracker.index}) =>
...
mfc.applyMixins(Countdown, [EventEmitter]);
